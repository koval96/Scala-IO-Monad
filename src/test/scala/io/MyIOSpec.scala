...
class MyIOSpec extends AnyFlatSpec with Matchers {
  ...
  "redeemWith" should "be stack-safe for large chains of error handling" in {
    @tailrec
    def recursiveRedeem(n: Int, acc: MyIO[Int] = MyIO.raiseError(exception)): MyIO[Int] =
      if (n > 0)
        recursiveRedeem(
          n - 1,
          acc.redeemWith(
    ...
    val result = recursiveRedeem(largeNumber).unsafeRunSync
    result shouldBe largeNumber
  }
